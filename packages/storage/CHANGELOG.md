# Changelog

## [1.1.14](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.14) - 2026-04-12

### Changed

- Bump package versions and fix description dashes — Franck ([78bf3699](https://github.com/ooneex/ooneex/commit/78bf3699))

## [1.1.12](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.12) - 2026-04-09

### Changed

- Change workspace deps from workspace:* to workspace:^ — Franck ([fe6fd3e1](https://github.com/ooneex/ooneex/commit/fe6fd3e1))

## [1.1.11](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.11) - 2026-04-08

### Changed

- Bump version to 1.1.10 — Franck ([6f1daf9b](https://github.com/ooneex/ooneex/commit/6f1daf9b))

## [1.1.9](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.9) - 2026-04-08

### Changed

- Bump all package versions — Franck ([3444f39f](https://github.com/ooneex/ooneex/commit/3444f39f))

## [1.1.7](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.7) - 2026-04-07

### Changed

- @ooneex/storage@1.1.6 — Franck ([5fc126bc](https://github.com/ooneex/ooneex/commit/5fc126bc))
- Add @types/bun to tsconfig types across all packages — Franck ([6faa8fe0](https://github.com/ooneex/ooneex/commit/6faa8fe0))
- Format BunnyStorage and FilesystemStorage exceptions — Franck ([44957ac6](https://github.com/ooneex/ooneex/commit/44957ac6))

## [1.1.6](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.6) - 2026-04-07

### Changed

- Add @types/bun to tsconfig types across all packages — Franck ([afd2c522](https://github.com/ooneex/ooneex/commit/afd2c522))
- Format BunnyStorage and FilesystemStorage exceptions — Franck ([1b8366b4](https://github.com/ooneex/ooneex/commit/1b8366b4))

## [1.1.5](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.5) - 2026-04-05

### Changed

- Simplify exception key names across all packages — Franck ([a621c033](https://github.com/ooneex/ooneex/commit/a621c033))
- Add exception key parameter across all packages — Franck ([b7c45c0c](https://github.com/ooneex/ooneex/commit/b7c45c0c))
- Format BunnyStorage method chains — Franck ([aaad5716](https://github.com/ooneex/ooneex/commit/aaad5716))
- Migrate BunnyStorage to official SDK — Franck ([fef061c2](https://github.com/ooneex/ooneex/commit/fef061c2))

## [1.1.4](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.4) - 2026-04-01

### Changed

- Disable minification in bunup configs across all packages — Franck ([29f840e1](https://github.com/ooneex/ooneex/commit/29f840e1))

### Removed

- Remove injectable decorator call from package decorators — Franck ([1d9af75b](https://github.com/ooneex/ooneex/commit/1d9af75b))

## [1.1.3](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.3) - 2026-04-01

### Changed

- Add injectable decorator call in all package decorators — Franck ([e07aa5f1](https://github.com/ooneex/ooneex/commit/e07aa5f1))

## [1.1.2](https://github.com/ooneex/ooneex/releases/tag/@ooneex/storage@1.1.2) - 2026-03-29

### Changed

- Remove Unreleased sections from changelogs — Franck ([971c3895](https://github.com/ooneex/ooneex/commit/971c3895))

## [1.1.1] - 2026-03-29

### Changed

- Bump versions and fix em-dash encoding in descriptions — Franck ([70391f14](https://github.com/ooneex/ooneex/commit/70391f14))
