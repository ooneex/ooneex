...
mock.module("@tanstack/ai-ollama", () => ({
  createOllamaChat: mock(() => ({ type: "ollama-adapter" })),
}));
...
describe("OllamaAi", () => {
  ...
  afterEach(() => {
    Bun.env.OLLAMA_HOST = originalHost;
  });

  describe("Host handling", () => {
    test("should use host from config", async () => {
      await ai.makeShorter("test content", { host: "http://custom-host:11434" });
  ...
  describe("model selection", () => {
    ...
    test("should use specified model from config", async () => {
      await ai.makeShorter("Text", { model: "mistral" });
