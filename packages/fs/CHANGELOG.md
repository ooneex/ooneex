# Changelog

## [1.1.13](https://github.com/ooneex/ooneex/releases/tag/@ooneex/fs@1.1.13) - 2026-04-12

### Changed

- Bump package versions and fix description dashes — Franck ([78bf3699](https://github.com/ooneex/ooneex/commit/78bf3699))

## [1.1.11](https://github.com/ooneex/ooneex/releases/tag/@ooneex/fs@1.1.11) - 2026-04-09

### Changed

- Change workspace deps from workspace:* to workspace:^ — Franck ([21b5df3f](https://github.com/ooneex/ooneex/commit/21b5df3f))

## [1.1.10](https://github.com/ooneex/ooneex/releases/tag/@ooneex/fs@1.1.10) - 2026-04-08

### Changed

- Bump version to 1.1.9 — Franck ([adc41b60](https://github.com/ooneex/ooneex/commit/adc41b60))

## [1.1.8](https://github.com/ooneex/ooneex/releases/tag/@ooneex/fs@1.1.8) - 2026-04-08

### Changed

- Bump all package versions — Franck ([3444f39f](https://github.com/ooneex/ooneex/commit/3444f39f))

## [1.1.6](https://github.com/ooneex/ooneex/releases/tag/@ooneex/fs@1.1.6) - 2026-04-07

### Changed

- @ooneex/fs@1.1.5 — Franck ([78c76b63](https://github.com/ooneex/ooneex/commit/78c76b63))
- Add @types/bun to tsconfig types across all packages — Franck ([6faa8fe0](https://github.com/ooneex/ooneex/commit/6faa8fe0))

## [1.1.5](https://github.com/ooneex/ooneex/releases/tag/@ooneex/fs@1.1.5) - 2026-04-07

### Changed

- Add @types/bun to tsconfig types across all packages — Franck ([afd2c522](https://github.com/ooneex/ooneex/commit/afd2c522))

## [1.1.4](https://github.com/ooneex/ooneex/releases/tag/@ooneex/fs@1.1.4) - 2026-04-05

### Changed

- Simplify exception key names across all packages — Franck ([a621c033](https://github.com/ooneex/ooneex/commit/a621c033))
- Add key parameter to exception constructors in tests — Franck ([64d0f50f](https://github.com/ooneex/ooneex/commit/64d0f50f))
- Add exception key parameter across all packages — Franck ([b7c45c0c](https://github.com/ooneex/ooneex/commit/b7c45c0c))

## [1.1.3](https://github.com/ooneex/ooneex/releases/tag/@ooneex/fs@1.1.3) - 2026-04-01

### Changed

- Disable minification in bunup configs across all packages — Franck ([29f840e1](https://github.com/ooneex/ooneex/commit/29f840e1))

## [1.1.2](https://github.com/ooneex/ooneex/releases/tag/@ooneex/fs@1.1.2) - 2026-03-29

### Changed

- Remove Unreleased sections from changelogs — Franck ([971c3895](https://github.com/ooneex/ooneex/commit/971c3895))

## [1.1.1] - 2026-03-29

### Changed

- Bump versions and fix em-dash encoding in descriptions — Franck ([70391f14](https://github.com/ooneex/ooneex/commit/70391f14))
