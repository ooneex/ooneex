...
describe("GeminiAi", () => {
  ...
  afterEach(() => {
    Bun.env.GEMINI_API_KEY = originalEnv;
  });
  ...
  describe("API Key handling", () => {
    ...
    test("should throw with descriptive message when API key is missing", async () => {
      ...
      try {
        ...
        expect((error as AiException).message).toContain("Gemini API key is required");
      }
    });
  });
  ...
  describe("model selection", () => {
    ...
    test("should use specified model from config", async () => {
      await ai.makeShorter("Text", { model: "gemini-2.0-flash" });
